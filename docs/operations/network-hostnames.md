...
애플리케이션 설정에는 IP 대신 아래 호스트명을 사용한다. IP fallback은
기본적으로 비워 두며, commissioning 중 필요하더라도 `192.168.30.x`만
명시적으로 허용한다.
...
별도 개발 PC에서 AI를 임시 실행할 때 production 호스트명을 재광고하지
말고, 해당 PC의 `192.168.30.x` 주소를 직접 진단용으로만 사용한다.
