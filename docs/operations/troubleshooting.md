...
3. **Main**: `main-server/scripts/real.sh`로 시작했는지, `smartfactory-main.local`이 이 PC의 로컬 `192.168.30.x` interface인지, `/health`, PostgreSQL 연결, Movement/AI URL, task/evidence/safety/recovery DB state를 확인한다. 실행기는 DB snapshot을 자동 복원하지 않는다.
