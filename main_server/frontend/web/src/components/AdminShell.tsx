...
const CONTEXT: Record<string, { eyebrow: string; title: string; description: string; checks: string[]; glyph: string }> = {
  ...
  "admin/devices": { eyebrow: "설비 구성", title: "로봇 & 카메라", description: "운용 대상 장치와 영상 소스, 통신 상태를 점검합니다.", checks: ["로봇 운용 여부", "카메라 귀속", "Movement 통신 로그"], glyph: "⬡" },
  "admin/system": { eyebrow: "플랫폼 진단", title: "시스템", description: "서비스 연결과 운영 데이터 저장 상태를 진단합니다.", checks: ["Main 연결", "Movement 모드", "DB 테이블 상태"], glyph: "⚙" },
  "records/events": { eyebrow: "감사 추적", title: "기록", description: "작업·장치 이벤트와 변경 이력을 시간순으로 확인합니다.", checks: ["위험 이벤트", "작업 이력", "통신·변경 근거"], glyph: "≡" },
};
export function AdminShell({ mode, currentRoute }: { mode: ModeDef; currentRoute: string }) {
  ...
  return <div className="admin-shell">
    <nav className="admin-activity-rail" aria-label="관리 영역"><div className="admin-rail-mark" aria-hidden="true">A</div><div className="admin-rail-destinations">{mode.items.map((item) => {
      const selected = item.key === active.key; const itemContext = CONTEXT[item.route] ?? CONTEXT["admin/system"];
      const shortLabel = item.label === "슬롯·재고·품목" ? "창고" : item.label.replace(" & 구역", "").replace("로봇·카메라", "장치");
      return <button key={item.key} type="button" className={selected ? "active" : ""} aria-current={selected ? "page" : undefined} aria-label={item.label} title={item.label} onClick={() => navigate(routePath(item.route))}><span className="admin-rail-glyph" aria-hidden="true">{itemContext.glyph}</span><span>{shortLabel}</span></button>;
    })}</div><button type="button" className="admin-rail-return" onClick={() => navigate("/operate/control")}><span aria-hidden="true">←</span><span>운영</span></button></nav>
