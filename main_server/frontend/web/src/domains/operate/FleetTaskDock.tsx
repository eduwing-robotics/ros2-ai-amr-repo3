...
import type { Robot, WorkOrder, WorkOrderRobotTask } from "../../types";
import { isTaskStepDone, taskStepLabel, taskStepVisualState } from "./taskProgressModel";
...
function TaskTimeline({ task }: { task: WorkOrderRobotTask }) {
...
export function FleetTaskDock({ robots, selectedRobotId, onRobotSelect }: { robots: Robot[]; selectedRobotId: string; onRobotSelect: (robotId: string) => void }) {
  ...
  return <section className="fleet-task-dock panel" aria-label="작업 큐, 할당 로봇, 타임라인과 안전 중지">
    ...
    <div className="fleet-task-rows">{rows.length === 0 ? <div className="empty">{tab === "queue" ? "진행·예약 작업이 없습니다." : "완료된 작업 기록이 없습니다."}</div> : rows.map(({ order, task }) => {
      ...
      return <article data-operation={order.operation} data-history-status={phase} data-status-tone={statusTone(taskStatus)} className={`fleet-task-row${selected ? " selected" : ""}${running ? " is-running" : ""}${tab === "history" ? " is-history" : ""}`} key={`${order.order_id}-${task?.task_id ?? "order"}`}>
        <div className="fleet-task-summary" title={"작업 #" + order.order_id + (task ? " · Task #" + task.task_id : "")}><Pill status={phase} /><strong>{running ? <span className="fleet-live-label"><i />LIVE</span> : null}{task ? "Task #" + task.task_id : "작업 #" + order.order_id}</strong><span className="operation-label"><OperationIcon operation={order.operation} />{order.operation === "inbound" ? "입고" : "출고"} · {order.item_code} · {task?.quantity ?? order.quantity}개</span>{Number(task?.priority ?? 0) > 0 ? <small>우선 {task?.priority}</small> : null}</div>
        ...
        <div className="fleet-task-progress">{task ? <TaskTimeline task={task} /> : <div className="fleet-idle-line"><span />작업 계획 대기</div>}</div>
