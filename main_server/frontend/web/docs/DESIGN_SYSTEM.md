...
- [UX 원칙](#1-ux-원칙)
- [운영 화면 정보 계층](#2-운영-화면-정보-계층)
- [디자인 토큰](#3-디자인-토큰)
- [컴포넌트 규격](#4-컴포넌트-규격)
- [반응형·접근성](#5-반응형접근성)
- [변경 체크리스트](#6-변경-체크리스트)
- [현재 목적지와 안전 상태](#7-현재-목적지와-안전-상태)
...
- Fleet task bar: 모든 로봇의 실행 작업, Movement 단계 진행, Work Order 안전 중지를 하단에 상시 유지
...
기준 팔레트는 Blue `#0673B9`, Cyan `#27BBD8`, Teal `#08A399`, Yellow `#F7D000`, Orange `#E84314`다. 원색은 출발점이며 작은 글자나 흰색 버튼에는 WCAG AA 대비를 만족하도록 어둡게 조정한 의미 토큰을 사용한다.

색상 적용 제약:

- 브랜드·운영 강조색은 위험 의미로 사용하지 않는다.
- warning·danger는 텍스트와 아이콘·점·테두리 중 하나를 반드시 병기한다.
- 확인된 이벤트는 기록에서 유지하되 강조만 낮춘다.
- 라이트·다크 테마는 같은 의미 매핑을 유지한다.
- ESTOP 활성은 danger, 정지·해제 미확인은 warning으로 표시하며 단순 offline과 합치지 않는다.
- 모든 화면은 `statusTone()`과 `--status-<tone>-*`를 사용하고 상태 코드를 별도 색으로 재정의하지 않는다.
...
- [관리자 워크스페이스 정책](ADMIN_WORKSPACE.md)
- [공개 UX 정본](../../../docs/UX.md)
- [UX 검수 항목](../../../docs/TEST_CASES.md)
