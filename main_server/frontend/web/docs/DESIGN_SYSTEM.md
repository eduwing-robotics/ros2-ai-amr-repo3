# AMR Control UI Policy

이 문서는 운영·관리 화면의 시각 언어와 상호작용 규칙을 정의한다. 새 화면과 기존 화면 개편은 이 문서를 기본 계약으로 사용한다.

## 1. UX 원칙

1. **관제 맥락 보존**: 지도, 전역 카메라, 연결 상태, E-STOP, 활성 작업 요약은 운영 중 사라지지 않는다.
2. **공간적 인과 일치**: 좌측 메뉴는 중앙/우측의 명시된 목적지 화면을 바꾸고, 우측 문맥 버튼은 우측 문맥 패널만 바꾼다. 상단 버튼이 하단 패널을 예고 없이 여는 패턴은 금지한다.
3. **예외 우선 강조**: 정상 상태는 중립색으로 표시하고 주의·위험·실행 중 상태에만 의미색을 사용한다.
4. **실행과 조회 분리**: 입출고·수동 조작은 우측 문맥 패널, 작업·재고·기록 조회는 하단 워크스페이스에 둔다.
5. **한 화면 한 기본 행동**: 각 패널의 기본 행동은 하나만 채움 버튼으로 표현하고 보조 행동은 외곽선 또는 텍스트 버튼을 사용한다.
6. **색 단독 전달 금지**: 모든 상태색에는 텍스트, 아이콘, 패턴 중 하나를 함께 제공한다.

## 2. 운영 화면 정보 계층

```text
┌──────────────┬───────────────────────────────────────────────┐
│ 목적지 내비 │ 전역 상태 · 연결 · 시간 · E-STOP              │
├──────────────┼───────────────────────────────┬───────────────┤
│ 관제        │ 제목 · 중요 알람              │ 문맥 패널      │
│ 입출고      ├───────────────────────────────┤ 로봇/입출고/조작│
│ 조작        │ 실시간 지도                    ├───────────────┤
│ 작업        │                               │ 전역 카메라    │
│ 재고        ├───────────────────────────────┴───────────────┤
│ 기록        │ 활성 작업 요약 / 조회 워크스페이스             │
├──────────────┤                                               │
│ 관리        │                                               │
└──────────────┴───────────────────────────────────────────────┘
```

### 상시 영역

- 전역 헤더: Main/Movement/Camera 연결, 로봇 연결 수, 시각, 테마, E-STOP
- 중요 알람: 미확인 위험·주의 이벤트 한 건과 전체 건수
- 실시간 지도: Pose 최신성, 로봇 위치, 경로, 구역, 목적지 지정
- 전역 카메라: 로봇 귀속이 없는 카메라, 전송 방식, 원본/오버레이
- 작업 요약: 활성·예약·진행·복구 건수

### 문맥 영역

- 기본: 로봇 상태, 배터리, 현재 작업, 개별 카메라
- 입출고: 유형, 품목, 수량, 존, 슬롯, 로봇 배정, 실행 전 검증
- 조작: 선택 로봇, Teleop, 맵 목적지 이동, Movement 차단 사유

### 조회·관리 영역

- 하단 워크스페이스: 작업, 재고, 기록
- 관리 모드: 맵·구역, 품목·슬롯·재고, 로봇·카메라, 시스템·DB

## 3. 디자인 토큰

### 색상

| 용도 | 토큰 | 라이트 값 | 규칙 |
|---|---|---:|---|
| 앱 배경 | `--bg` | `#f4f6f8` | 콘텐츠 캔버스 |
| 기본 표면 | `--surface` | `#ffffff` | 패널·입력 |
| 보조 표면 | `--surface-2` | `#f8fafc` | 행·도구막대 |
| 본문 | `--text` | `#172033` | 핵심 정보 |
| 보조 본문 | `--text-muted` | `#667085` | 설명·메타 |
| 테두리 | `--border` | `#d9e0e8` | 기본 1px |
| 운영 강조 | `--operator` | `#315fe9` | 선택·기본 행동 |
| 정상 | `--ok-*` | 녹색 계열 | 연결점과 정상 확인에만 사용 |
| 주의 | `--warn-*` | 호박 계열 | 지연·확인 필요 |
| 위험 | `--err-*` | 적색 계열 | E-STOP·실패·파괴 행동 |

정상 패널 전체를 녹색으로 칠하지 않는다. 상태색 배경은 저채도 의미 토큰만 사용한다.

### 간격·크기

- 기본 간격 단위: 4px
- 허용 간격: 4, 8, 12, 16, 24, 32px
- 화면 여백: 데스크톱 20~24px, 좁은 화면 12px
- 패널 내부: 12~16px
- 제어 높이: 소형 32px, 기본 38~40px, 위험 전역 행동 40px 이상
- 내비게이션: 데스크톱 216px, 우측 문맥 레일 390px
- 전역 카메라: 데스크톱 최소 220px 높이, 문맥 패널과 독립

### 모서리·테두리·그림자

- 소형 요소: `--radius-sm: 7px`
- 입력·버튼: `--radius: 9px`
- 패널: `--radius-lg: 12px`
- 기본 테두리: 1px, 선택 강조는 2px 외곽선 또는 3px 인셋 바
- 그림자는 계층 분리가 필요한 팝오버·드로어에만 사용하고 기본 패널은 테두리 중심으로 구분한다.

### 타이포그래피

- UI: system-ui, Pretendard, Noto Sans KR 순서
- 화면 제목 24px/700, 패널 제목 15px/700, 본문 13~14px, 메타 11~12px
- ID, 좌표, 시각, 퍼센트는 `.mono`와 tabular numerals를 사용한다.
- 영문 대문자 라벨은 11px 이하에서만 사용하고 자간을 넓힌다.

## 4. 컴포넌트 규격

### 버튼

- Primary: 현재 패널의 단일 완료 행동
- Secondary: 취소·새로고침·보기 전환
- Ghost: 접기·확대·세부 이동
- Danger: E-STOP·안전 중단·삭제
- 비활성 버튼은 사유 텍스트 또는 툴팁을 함께 제공한다.

### 패널

- 헤더 높이 48~56px, 제목 왼쪽, 상태·필터는 오른쪽
- 패널 내부 스크롤은 헤더와 주요 실행 버튼을 고정한 뒤 본문에만 적용한다.
- 패널을 여는 트리거와 결과 패널은 같은 축에 배치한다.

### 알람

- 위험: 적색 좌측 3px + 명시적 위험 텍스트
- 주의: 호박 좌측 3px + 확인 필요 텍스트
- 정보: 중립 배경, 별도 강조 테두리 없음
- 확인한 알람은 목록에서 제거하지 않고 강조만 낮춘다.

### 카메라

- 전역 카메라는 관제 화면에서 항상 렌더링한다.
- 로봇 카메라는 선택 로봇 문맥에서 제공한다.
- WebRTC 우선, MJPEG fallback 상태를 텍스트 배지로 표시한다.
- 영상이 없을 때 검은 빈 박스를 유지하지 않고 오프라인 사유와 재시도를 표시한다.

### 표와 폼

- 표 헤더는 고정하고 상태 열과 행동 열의 위치를 화면 간 통일한다.
- 입력 라벨은 항상 노출하며 placeholder를 라벨 대신 사용하지 않는다.
- 긴 폼은 단계 번호와 섹션 제목으로 나누고 실행 버튼은 하단에 고정한다.

## 5. 반응형·접근성

- 1200px 초과: 지도 + 390px 문맥 레일 + 내비게이션
- 901~1200px: 문맥 패널은 모달 드로어, 전역 카메라는 지도 아래 유지
- 900px 이하: 조회 워크스페이스와 지도는 세로 적층
- 모든 조작은 키보드 접근 가능해야 하며 `:focus-visible` 외곽선을 제거하지 않는다.
- 모달은 focus trap, Escape 닫기, 트리거 focus 복귀를 제공한다.
- 최소 텍스트 대비는 WCAG AA를 목표로 한다.

## 6. 변경 체크리스트

- [ ] 지도·전역 카메라·E-STOP이 핵심 작업 중 유지되는가
- [ ] 트리거와 변경되는 영역이 공간적으로 일치하는가
- [ ] 정상 상태에 과도한 의미색을 쓰지 않았는가
- [ ] 로딩·빈 상태·오류·비활성 사유가 있는가
- [ ] 1920×1080, 1440×900, 1000×800에서 핵심 기능을 사용할 수 있는가
- [ ] 키보드 포커스와 ARIA 이름이 유지되는가
- [ ] TypeScript, lint, Playwright 핵심 UX 테스트를 통과하는가
