...
운영·관리 전환은 공통 헤더의 모드 탭에서만 제공한다. 운영 및 관리 좌측 탐색에는 반대 모드로 이동하는 중복 버튼이 없어야 하며, 좌측 탐색은 현재 모드 내부 목적지만 포함한다.

## WEB-22 해상도별 적응형 레이아웃

아래 화면 매트릭스에서 헤더, 현재 모드 탐색, 중앙 작업면과 핵심 명령은 생략되지 않아야 한다. 문서 전체에 가로 스크롤이 생기지 않아야 하며, 내용이 최소 크기를 넘으면 해당 패널이 스크롤을 소유한다.
