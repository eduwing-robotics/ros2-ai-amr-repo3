-- PostgreSQL MVP schema — database/dbml/smartfactory-db-final.dbml
-- CHECK/default policies from  minimum supplement.
...
-- : widen locations.type CHECK on existing databases (idempotent).
...
-- Single-map runtime: every location belongs to the only filesystem map.
DROP INDEX IF EXISTS idx_locations_map_id;
ALTER TABLE locations DROP COLUMN IF EXISTS map_id;
