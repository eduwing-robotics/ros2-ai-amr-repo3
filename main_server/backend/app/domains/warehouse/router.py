...
from app.db.postgres import DEFAULT_FLOOR, event_repo, inventory_repo, item_repo, location_repo
...
def upsert_item(payload: ItemUpsert) -> ApiMessage:
    """품목을 생성하거나 수정한다."""
    with transaction() as conn:
        item_repo.upsert(conn, payload.model_dump())
        event_repo.append(
            conn,
...
def delete_item(item_code: str) -> ApiMessage:
    """품목을 삭제한다."""
    with transaction() as conn:
        deleted = item_repo.delete(conn, item_code)
        if not deleted:
            raise HTTPException(status_code=404, detail="item not found")
        event_repo.append(conn, event_type="DB_ITEM_DELETE", message=f"item deleted: {item_code}")
...
def list_storage_slots() -> list[StorageSlot]:
    ...
    with transaction() as conn:
        return [StorageSlot(**slot) for slot in location_repo.list_locations(conn, "storage")]
...
def upsert_storage_slot(payload: StorageSlotUpsert) -> ApiMessage:
    ...
    with transaction() as conn:
        location_repo.upsert(conn, payload.model_dump())
        event_repo.append(
            conn,
...
def delete_storage_slot(slot_id: str) -> ApiMessage:
    ...
    with transaction() as conn:
        deleted = location_repo.delete(conn, slot_id)
        if not deleted:
            raise HTTPException(status_code=404, detail="storage slot not found")
        event_repo.append(conn, event_type="DB_STORAGE_SLOT_DELETE", message=f"storage slot deleted: {slot_id}")
...
def list_inventory(
    ...
    with transaction() as conn:
        rows = inventory_repo.list(conn, slot_id=slot_id, item_code=item_code, floor=floor)
...
def upsert_inventory(payload: InventoryUpsert) -> ApiMessage:
    """슬롯별 품목 수량 생성/수정."""
    with transaction() as conn:
        if not item_repo.exists(conn, payload.item_code):
            raise HTTPException(status_code=404, detail="item not found")
        slot = location_repo.get(conn, payload.slot_id)
        ...
        if payload.quantity > 0:
            rows = inventory_repo.list(conn, slot_id=payload.slot_id, floor=floor)
        ...
        inventory_repo.upsert(conn, {**payload.model_dump(), "floor": floor})
        event_repo.append(
            conn,
