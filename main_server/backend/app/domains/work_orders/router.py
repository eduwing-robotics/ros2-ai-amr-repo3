...
from app.domains.work_orders import planner, projections, workflow
...
def list_work_orders(
    ...
    with transaction() as conn:
        return [WorkOrder(**order) for order in projections.list_work_orders(conn, limit=limit, status=status)]
...
def get_work_order(order_id: int) -> WorkOrder:
    """입고/출고 업무 요청 상세."""
    with transaction() as conn:
        return WorkOrder(**projections.get_work_order(conn, order_id))
...
def cancel_work_order(order_id: int) -> WorkOrder:
    ...
    with write_transaction() as conn:
        return WorkOrder(**workflow.cancel_work_order(conn, order_id))
...
def stop_work_order(order_id: int) -> dict:
    ...
    with write_transaction() as conn:
        return workflow.request_work_order_stop(conn, order_id)
...
def set_work_order_priority(order_id: int, payload: WorkOrderPriorityUpdate) -> WorkOrder:
    ...
    with write_transaction() as conn:
        return WorkOrder(**workflow.set_work_order_priority(conn, order_id, payload.priority))
...
def preview_work_order(payload: WorkOrderPreviewRequest) -> WorkOrderPreview:
    ...
    with transaction() as conn:
        return WorkOrderPreview(**planner.preview_work_order(conn, payload.model_dump()))
