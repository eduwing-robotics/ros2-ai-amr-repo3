...
def dispatch_robot_command(conn, payload: RobotCommandRequest, request: Request | None = None) -> RobotCommandResponse:
    ...
    if payload.kind == "inout_scenario":
        return _dispatch_inout_scenario(payload, command_id, callback_url)
...
def _send_inout_scenario_command(
    robot_id: str,
    body: dict[str, Any],
) -> tuple[dict[str, Any], bool]:
    """불확실한 POST 실패에는 같은 command ID 조회 후 404에서만 동일 body를 재전송한다."""
    try:
        return movement_client.inout_scenario_command(robot_id, body), False
    except MovementClientError as post_error:
        if post_error.status_code is not None:
            raise
        try:
            status = movement_client.inout_scenario_status(robot_id, str(body["command_id"]))
...
def _dispatch_inout_scenario(
    ...
    if payload.dry_run:
        return RobotCommandResponse(
            ...
            dry_run=True,
            accepted=True,
            response={"validated": True, "step_count": 9, "request_body": body},
        )
    try:
        response, recovered = _send_inout_scenario_command(payload.robot_id, body)
    except (NotImplementedError, MovementClientError) as exc:
        if isinstance(exc, MovementClientError):
            raise _map_movement_client_error(exc, kind="inout_scenario") from exc
        raise HTTPException(status_code=501, detail="movement_scenario_commands_api_missing") from exc
    mismatches: list[str] = []
    expected = {
        "command_id": command_id,
        "task_id": payload.task_id,
    ...
    if not recovered:
        expected["scenario_type"] = body["scenario_type"]
    ...
    if not recovered and owner != "MOVEMENT":
        mismatches.append("authority_owner")
    if mismatches:
        raise HTTPException(
            status_code=502,
            detail={"code": "inout_scenario_acceptance_contract_mismatch", "fields": sorted(set(mismatches))},
        )
    return RobotCommandResponse(
        command_id=command_id,
        robot_id=payload.robot_id,
        ...
        dry_run=False,
        accepted=True,
        response={**response, "recovered_after_timeout": recovered, "step_count": 9},
    )
