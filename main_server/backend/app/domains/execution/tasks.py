...
def robot_assignment_block_reason(robot_id: str) -> str | None:
    ...
    # Main의 기존 정수 퍼센트 계약을 그대로 사용한다. 미수신(null)은 저전력으로
    # 오판하지 않고, 20% 미만만 신규 작업 배정에서 제외한다.
...
def auto_assign_and_start(
    ...
    """유휴 로봇에 배정 후 곧바로 Task 실행까지 수행한다(sweeper 등 백그라운드용).
    ...
    for entry in result["assigned"]:
        ...
        try:
            start_task_execution(conn, task_id, callback_base_url, source)
