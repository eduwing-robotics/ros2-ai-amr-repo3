...
#
# 사용법:
#   ./scripts/launch/real.sh              # Main만 (:8088, dist 있으면 정적 서빙)
#   ./scripts/launch/real.sh --dev        # Main + Vite dev (:5173)
#   ./scripts/launch/real.sh --reload     # uvicorn auto-reload
#   ./scripts/launch/real.sh --build      # 프론트 빌드 후 Main 실행
#
# PostgreSQL: .env 의 LMS_DATABASE_URL 필수. 최초 1회 ./scripts/db/setup_pg.sh
# 종료: ./scripts/launch/real.sh --stop
# UI mock(dry)는 ./scripts/launch/fake.sh — fake API + Vite.
set -euo pipefail
...
source "$ROOT/scripts/launch/lib/pg_bootstrap.sh"
pg_ensure_running
