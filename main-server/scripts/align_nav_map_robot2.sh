...
# robot2_map 전용 zones/bindings/seed audit이 완료되기 전에는 canonical field
# task dispatch가 HTTP 409으로 차단된다; 이 스크립트는 map alignment만 수행한다.
#
...
#   2) Nav 적용 후 sync-from-movement 검증 안내
#
# 사용법:
...
")"
[[ "$has_target" == "yes" ]] || fail "Main DB has no map_id=$TARGET_MAP"
...
if [[ -z "$nav_state" ]]; then
  ...
  if [[ "$active" == "$TARGET_MAP" ]]; then
    ...
    info "Run on Main: curl -X POST $API_BASE/maps/sync-from-movement"
...
cat <<EOF
curl -X POST $API_BASE/maps/sync-from-movement
