...
- **성숙도(사실/목표/Draft)는 폴더가 아니라 `상태:` 메타로 표시한다.** 같은 폴더 안에서 구현된 사실은 `상태: Active`, 미구현 목표는 `상태: Draft`로 구분한다(예: `ui-ux/FRONTEND.md`=Active, `ui-ux/pages/admin-actions.md`=Draft).
