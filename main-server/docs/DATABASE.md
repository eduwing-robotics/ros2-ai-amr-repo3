...
![데이터 관계도](<../../assets/Images/Screenshot from 2026-07-23 10-27-24.png>)
...
### 작업 완료와 재고를 함께 저장하는 이유

로봇이 화물을 들어 올렸다는 물리 결과만으로 재고를 바로 변경하지 않습니다. Main은 Nav 결과가 현재 작업·로봇·단계·`command_id`와 일치하고, 필요한 AI evidence까지 유효한지 확인한 뒤 업무 완료를 확정합니다.

예를 들어 입고 수량만 증가하고 작업 완료 기록이 실패하면 실제 화물과 작업 큐의 상태가 서로 달라집니다. 반대로 작업만 완료되고 재고가 반영되지 않아도 다음 출고 계획이 잘못됩니다. 이를 막기 위해 재고 수량, 변경 이력, 작업 완료 기록과 로봇 해제를 같은 transaction에서 처리하며, 하나라도 실패하면 전체 변경을 Rollback합니다.
