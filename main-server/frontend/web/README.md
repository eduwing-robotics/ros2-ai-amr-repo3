...
- **관제 화면**에서는 지도 위 로봇 위치와 연결 상태, 카메라, 활성 작업과 안전 경고를 함께 확인합니다. 정보가 stale이거나 외부 서버가 오프라인이면 화면에서 성공 상태로 보정하지 않고 마지막 확인 시각과 이상 상태를 표시합니다.
- **작업 화면**에서는 작업 큐와 현재 실행 단계를 확인하고, 실패·보류된 작업에 대해 재개, 안전 위치 이동 또는 중단을 선택합니다. 브라우저는 이전 명령을 직접 재전송하지 않고 Main의 복구 API를 통해 새 실행 여부를 결정합니다.
- **시스템 화면**에서는 Main·PostgreSQL·Nav·AI의 연결과 설정을 진단합니다. 이 화면의 연결 표시는 물리 작업 완료를 의미하지 않으며, 실물 명령 전에는 로봇 readiness와 현재 작업 조건을 별도로 확인합니다.
