...
import { WorkOrderQueue } from "./TaskQueue";
...
export function OperatorShell() {
  ...
  const allRobotsEmergency = enabledRobots.length > 0 && enabledRobots.every((robot) => emergencyRobots.includes(robot.robot_id));
  const selectedRobotOperational = selectedRobot?.enabled === true;
              ...
              {drawer === "control" ? (
                <>
                  ...
                  <div className="operator-control-bar-inner">
                    <Teleop
                      robots={selectedRobotOperational && selectedRobot ? [selectedRobot] : []}
                      disabled={!movementAvailable || !selectedRobotOperational}
                      isRobotEmergency={isRobotEmergency}
                      keyboardEnabled={drawer === "control"}
                    />
                    <MapGotoOperate robots={selectedRobotOperational && selectedRobot ? [selectedRobot] : []} disabled={!movementAvailable || !selectedRobotOperational} isRobotEmergency={isRobotEmergency} />
                  </div>
                </>
              ) : null}
            </Drawer>
