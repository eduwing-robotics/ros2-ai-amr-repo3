...
export function MapGoto({ robots }: { robots: Robot[] }) {
  ...
  useEffect(() => {
    ...
    setMapId((cur) => (cur && maps.some((m) => m.map_id === cur) ? cur : maps[0].map_id));
  }, [maps]);
  ...
  const runInitialPose = async () => {
    ...
    try {
      ...
      if (msg.includes("movement_initial_pose_api_missing")) {
        setStatus("Movement initial pose API 미구현 (movement_initial_pose_api_missing)");
      } else {
  ...
  return (
    <div className="panel">
      ...
      <div className="toolbar">
        ...
        <select className="filter" value={map?.map_id ?? ""} onChange={(e) => setMapId(e.target.value)}>
          {maps.length === 0 ? <option value="">맵 없음</option> : maps.map((m) => <option key={m.map_id} value={m.map_id}>{m.name}</option>)}
        </select>
        <input className="search mono" type="number" step="1" title="도착 방향(도)" placeholder="방향(°)" value={yawDeg} onChange={(e) => setYawDeg(e.target.value)} style={{ width: 72 }} />
