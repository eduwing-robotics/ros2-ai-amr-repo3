"""PHASE_74 — bootstrap vs demo seed persistence and map_id filter tests."""
...
from app.db.mvp_repositories import MvpLocationRepository
...
class SeedPersistenceTest(unittest.TestCase):
    ...
    def test_waypoints_filtered_by_map_id(self) -> None:
        ...
        self.assertNotIn("MAP_B_ONLY", ids_a)
        self.assertIn("MAP_B_ONLY", ids_b)
        self.assertNotIn("MAP_A_ONLY", ids_b)
    ...
    def test_waypoint_returns_row_map_id(self) -> None:
        with write_transaction() as conn:
            locs = MvpLocationRepository(conn)
            locs.upsert_waypoint({
        ...
        self.assertEqual(found["map_id"], "custom_map")
