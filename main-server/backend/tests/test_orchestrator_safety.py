...
class PollRunningTasksGateTest(unittest.TestCase):
    ...
    def test_poll_reconciles_canceled_command_when_callback_is_missing(self) -> None:
        conn = MagicMock()
        task = {
            "task_id": 10,
            "assigned_robot_id": "robot1",
        ...
        with (
            ...
            advance.return_value = {"task_id": 10}
        ...
        advance.assert_called_once_with(
            conn,
            10,
            {"command_id": "cmd-cancelled", "state": "CANCELED"},
    ...
    def test_poll_reconciles_unconfirmed_stop_after_main_restart(self) -> None:
        ...
        with (
            ...
            evidence_runtime.list_orchestrated_running.return_value = [task]
            movement_client.command_status.return_value = {"state": "STOP_UNCONFIRMED"}
            advance.return_value = {"task_id": 11}
        ...
        advance.assert_called_once_with(
            conn,
            11,
            {"command_id": "cmd-stop-unknown", "state": "STOP_UNCONFIRMED"},
            source="task_progress_poller",
        )
