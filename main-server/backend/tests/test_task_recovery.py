...
class TaskRecoveryTest(unittest.TestCase):
    ...
    def _execute_safe_replan(self, *, stops, health):
        conn = MagicMock()
        evidence = MagicMock()
        ...
        with (
            ...
            with self.assertRaises(HTTPException) as ctx:
                recovery.execute_recovery(conn, 1, cargo_state="LOADED", strategy="safe_replan", checks={"area_clear": True})
        dispatch.assert_not_called()
        return ctx.exception

    def test_all_true_checks_do_not_dispatch_with_open_task_safety_stop(self) -> None:
        error = self._execute_safe_replan(
    ...
    def test_all_true_checks_do_not_dispatch_when_live_health_fails(self) -> None:
        error = self._execute_safe_replan(stops=[], health=RuntimeError("health unavailable"))
    ...
    def test_all_true_checks_do_not_dispatch_when_live_health_is_ambiguous(self) -> None:
        error = self._execute_safe_replan(stops=[], health={"r1": {"ok": True}})
        self.assertEqual(error.detail, "recovery_live_health_unsafe")
    ...
    def test_unknown_cargo_blocks_preview(self) -> None:
        ...
        with (
            ...
            recovery.preview_recovery_plan(conn, 1, cargo_state="UNKNOWN", strategy="safe_replan")
    ...
    def test_loaded_preview_includes_safe_move(self) -> None:
        ...
        with (
            ...
            plan = recovery.preview_recovery_plan(conn, 1, cargo_state="LOADED", strategy="safe_replan")
