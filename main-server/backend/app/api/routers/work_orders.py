...
@router.post(
    "/work-orders/{order_id}/stop",
    response_model=WorkOrderStopResult,
    status_code=status.HTTP_202_ACCEPTED,
)
def stop_work_order(order_id: int) -> WorkOrderStopResult:
    """Request an idempotent safe stop for a running work order."""
    with write_transaction() as conn:
        return WorkOrderStopResult(**work_order_service.stop_work_order(conn, order_id))
