"""Task queue and assignment routes."""

from __future__ import annotations

from fastapi import APIRouter, Depends, Query, Request

from app.api.helpers import callback_base_url
from app.db.connection import transaction
from app.db.repo_bridge import task_repo
from app.models.schemas import MissionStatusResponse, Task, TaskAssign, TaskCreate
from app.security import require_operator
from app.services import task_recovery as recovery_service
from app.services import tasks as task_service

router = APIRouter(tags=["tasks"])


@router.get("/tasks", response_model=list[Task])
def list_tasks(status: str | None = Query(default=None), limit: int = Query(default=50, ge=1, le=200)) -> list[Task]:
    """작업 목록(최근순). status로 필터 가능."""
    with transaction() as conn:
        return [Task(**t) for t in task_repo(conn).list(limit=limit, status=status)]


@router.post("/tasks", response_model=Task, dependencies=[Depends(require_operator)])
def create_task(payload: TaskCreate) -> Task:
    """작업을 생성한다(QUEUED)."""
    with transaction() as conn:
        return Task(**task_service.create_task(conn, payload.model_dump()))


@router.post("/tasks/{task_id}/assign", response_model=Task, dependencies=[Depends(require_operator)])
def assign_task(task_id: int, payload: TaskAssign) -> Task:
    """작업을 특정 로봇에 수동 배정한다(유휴 로봇만)."""
    with transaction() as conn:
        return Task(**task_service.assign_task(conn, task_id, payload.robot_id))


@router.post("/tasks/{task_id}/start-mission", dependencies=[Depends(require_operator)])
def start_task_mission(task_id: int, request: Request) -> dict:
    """ASSIGNED 작업의 snapshot을 Movement mission으로 시작한다."""
    resolved_callback = callback_base_url(request)
    with transaction() as conn:
        result = task_service.start_task_mission(conn, task_id, callback_base_url=resolved_callback)
    return {
        "task": Task(**result["task"]),
        "mission": MissionStatusResponse(
            robot_id=result["robot_id"],
            command_id=result.get("command_id"),
            response={
                "leg_count": result.get("leg_count"),
                "command_id": result.get("command_id"),
            },
        ),
    }


@router.post("/tasks/{task_id}/complete", response_model=Task, dependencies=[Depends(require_operator)])
def complete_task(task_id: int) -> Task:
    """작업을 완료 처리하고 로봇을 IDLE로 되돌린다."""
    with transaction() as conn:
        return Task(**task_service.complete_task(conn, task_id))


@router.post("/tasks/{task_id}/cancel", response_model=Task, dependencies=[Depends(require_operator)])
def cancel_task(task_id: int) -> Task:
    """작업을 취소 처리하고 로봇을 IDLE로 되돌린다."""
    with transaction() as conn:
        return Task(**task_service.cancel_task(conn, task_id))


@router.post("/tasks/auto-assign", dependencies=[Depends(require_operator)])
def auto_assign_tasks() -> dict:
    """배정 대기 작업을 유휴·준비된 로봇에 우선순위 순으로 자동 배정한다(시작은 안 함)."""
    with transaction() as conn:
        return task_service.auto_assign(conn)


@router.post("/tasks/auto-assign-and-start", dependencies=[Depends(require_operator)])
def auto_assign_and_start_tasks(request: Request) -> dict:
    """자동 배정 후 배정된 작업의 미션까지 즉시 시작한다(개별 시작 실패는 기록만)."""
    resolved_callback = callback_base_url(request)
    with transaction() as conn:
        return task_service.auto_assign_and_start(conn, callback_base_url=resolved_callback)


@router.get("/tasks/recovery/needs-attention")
def list_recovery_tasks(limit: int = Query(default=20, ge=1, le=100)) -> list[dict]:
    with transaction() as conn:
        return recovery_service.list_needs_attention_tasks(conn, limit=limit)


@router.get("/tasks/{task_id}/recovery/context")
def recovery_context(task_id: int) -> dict:
    with transaction() as conn:
        return recovery_service.get_recovery_context(conn, task_id)


@router.post("/tasks/{task_id}/recovery/preview", dependencies=[Depends(require_operator)])
def recovery_preview(task_id: int, body: dict) -> dict:
    with transaction() as conn:
        return recovery_service.preview_recovery_plan(
            conn,
            task_id,
            cargo_state=body.get("cargo_state", "UNKNOWN"),
            strategy=body.get("strategy", "safe_replan"),
        )


@router.post("/tasks/{task_id}/recovery/decision", dependencies=[Depends(require_operator)])
def recovery_decision(task_id: int, body: dict) -> dict:
    with transaction() as conn:
        return recovery_service.save_recovery_decision(
            conn,
            task_id,
            cargo_state=body.get("cargo_state", "UNKNOWN"),
            strategy=body.get("strategy", "safe_replan"),
            checks=body.get("checks") or {},
        )


@router.post("/tasks/{task_id}/recovery/execute", dependencies=[Depends(require_operator)])
def recovery_execute(task_id: int, request: Request, body: dict) -> dict:
    with transaction() as conn:
        from app.api.helpers import callback_base_url
        return recovery_service.execute_recovery(
            conn,
            task_id,
            cargo_state=body.get("cargo_state", "UNKNOWN"),
            strategy=body.get("strategy", "safe_replan"),
            checks=body.get("checks") or {},
            callback_base_url=callback_base_url(request),
        )
