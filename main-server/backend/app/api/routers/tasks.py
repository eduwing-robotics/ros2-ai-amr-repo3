...
def recovery_decision(task_id: int, body: RecoveryActionRequest) -> dict:
    with transaction() as conn:
        return recovery_service.save_recovery_decision(
            conn,
            task_id,
            cargo_state=body.cargo_state,
...
def recovery_execute(task_id: int, body: RecoveryActionRequest) -> dict:
    with transaction() as conn:
