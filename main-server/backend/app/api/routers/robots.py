...
@router.post("/robots", response_model=ApiMessage, dependencies=[Depends(require_admin)])
def upsert_robot(payload: RobotUpsert) -> ApiMessage:
    """DB 관리 화면에서 로봇을 생성하거나 수정한다."""
    with transaction() as conn:
        robot_repo(conn).upsert(payload.model_dump())
...
@router.delete("/robots/{robot_id}", response_model=ApiMessage, dependencies=[Depends(require_admin)])
