"""Robot command envelope dispatch (PHASE_12-B).
...
def resolve_callback_url(request: Request | None = None, override: str | None = None) -> str:
    """Return only the server-configured callback endpoint."""
    del request, override
    return command_events_callback_url()
...
def _dispatch_move_to_point(conn, payload: RobotCommandRequest, command_id: str, callback_url: str) -> RobotCommandResponse:
    ...
    # Coordinate commands are map-frame data.  Unlike legacy manual UI flows,
    # dispatch must not silently substitute the robot's active map id.
    map_state = field_bindings.assert_robot_live_map(payload.robot_id, lms_map_id)
    map_context = {
        "ui_map_id": lms_map_id,
        "runtime_map_id": lms_map_id,
        "runtime_confidence": map_state.get("confidence", "live"),
    }
    ...
    event_repo(conn).append(
        ...
        robot_id=payload.robot_id,
        command_id=result.command_id,
        message=f"ui_map={lms_map_id} runtime_map={lms_map_id}",
        payload=map_context,
    )
    return result
...
def normalize_dock_transfer_params(
    ...
    *,
    ...
    """dock_transfer params 정규화 — 필수 3필드 + 선택 lift override 화이트리스트(PHASE_51)."""
...
def _dispatch_passthrough(payload: RobotCommandRequest, command_id: str, callback_url: str) -> RobotCommandResponse:
    """PHASE_18-A — 네이티브 POST /robot-commands envelope 패스스루."""
