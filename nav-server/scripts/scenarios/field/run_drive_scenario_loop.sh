...
#
# Usage:
#   bash scripts/scenarios/field/run_drive_scenario_loop.sh
#   LOOPS=2 SCENARIOS="in2_b_wait2 in1_c_wait2" bash scripts/scenarios/field/run_drive_scenario_loop.sh
#   PARK_MODE=nav_only bash scripts/scenarios/field/run_drive_scenario_loop.sh   # 대기장 nav만 (hold 생략)
#
set -euo pipefail
...
for (( loop=1; loop<=LOOPS; loop++ )); do
  ...
  for scenario in $SCENARIOS; do
    ...
    if ! SCENARIO="$scenario" LOOPS=1 PARK_MODE="$PARK_MODE" bash "$ROOT/scripts/scenarios/field/run_drive_insert_scenario.sh"; then
