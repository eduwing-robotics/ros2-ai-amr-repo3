#!/usr/bin/env bash
# outbound2 → insert/reverse → A슬롯 → insert/reverse → 대기2 hold
set -euo pipefail

BASE="${MOVEMENT_API_URL:-http://127.0.0.1:8002}"
ROBOT_ID="${ROBOT_ID:-tb3_2}"
TASK_ID="${TASK_ID:-9020}"
TS=$(date +%s)
...
# 1) 출고2 approach (Nav2 + approach yaw rotate + full align)
C1="ob2a-${TS}-out2"
post "$C1" "move_to_point" '{"waypoint_id":"outbound_slot_2_approach"}'
poll "$C1" "ARRIVED" 480

# 2) 출고2 dock: align + insert + reverse (lift no-op)
C2="ob2a-${TS}-out2dock"
post "$C2" "dock_transfer" '{"aruco_marker_id":6,"action":"load","level":1}'
poll "$C2" "DONE" 300

# 3) A슬롯 approach
C3="ob2a-${TS}-a"
post "$C3" "move_to_point" '{"waypoint_id":"warehouse_a_approach"}'
poll "$C3" "ARRIVED" 480

# 4) A슬롯 dock
C4="ob2a-${TS}-adock"
post "$C4" "dock_transfer" '{"aruco_marker_id":7,"action":"load","level":1}'
poll "$C4" "DONE" 300
