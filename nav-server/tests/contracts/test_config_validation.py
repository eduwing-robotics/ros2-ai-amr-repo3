...
def test_aruco_observation_sources_keep_http_alignment_and_local_detector_contract():
    ...
    robots = json.loads(
        (NAV_SERVER_ROOT / "config" / "robots.json").read_text()
    )["robots"]
...
def test_robot2_map_field_dispatch_is_commissioned_with_physical_lift():
    ...
    robot = next(item for item in json.loads((NAV_SERVER_ROOT / "config" / "robots.json").read_text())["robots"] if item["robot_id"] == "tb3_burger_02")
...
def test_both_robots_use_stationary_global_search_without_continuous_gate():
    ...
    config_dir = NAV_SERVER_ROOT / "config"
