...
def _configured_callback_url(url: str) -> str | None:
    ...
    if parsed_base.scheme == "http" and not (
        nohardware_allowed or field_lan_allowed or _is_site_main_host(
            parsed_base.hostname or "", parsed_base.port or 80
        )
    ):
        return None
    ...
    if parsed.scheme != parsed_base.scheme or parsed.netloc != parsed_base.netloc:
        return None
    if not nohardware_allowed and not field_lan_allowed and not _is_site_main_host(
