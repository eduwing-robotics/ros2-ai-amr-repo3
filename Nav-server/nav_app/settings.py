...
MOVEMENT_STATE_PATH = Path(
    os.getenv("MOVEMENT_STATE_PATH", ROOT / "worklog" / f"movement_state_{ACTIVE_ROBOT_ID}.json")
)
...
MOVEMENT_CALLBACK_TOKEN = os.getenv("LMS_MOVEMENT_CALLBACK_TOKEN", "")
CALLBACK_MAX_ATTEMPTS = max(1, int(os.getenv("MAIN_CALLBACK_MAX_ATTEMPTS", "3")))
CALLBACK_RETRY_BASE_SEC = max(0.0, float(os.getenv("MAIN_CALLBACK_RETRY_BASE_SEC", "0.25")))
ROBOT_STATUS_HEARTBEAT_SEC = max(0.5, float(os.getenv("ROBOT_STATUS_HEARTBEAT_SEC", "2.0")))
BATTERY_STALE_SEC = max(1.0, float(os.getenv("BATTERY_STALE_SEC", "5.0")))
BATTERY_WARNING_PERCENT = float(os.getenv("BATTERY_WARNING_PERCENT", "30.0"))
BATTERY_CRITICAL_PERCENT = float(os.getenv("BATTERY_CRITICAL_PERCENT", "20.0"))
BATTERY_EMPTY_VOLTAGE = float(os.getenv("BATTERY_EMPTY_VOLTAGE", "10.8"))
BATTERY_FULL_VOLTAGE = float(os.getenv("BATTERY_FULL_VOLTAGE", "12.6"))
BATTERY_FILTER_WINDOW = max(3, int(os.getenv("BATTERY_FILTER_WINDOW", "15")))
BATTERY_RISE_STEP_PERCENT = max(0.1, float(os.getenv("BATTERY_RISE_STEP_PERCENT", "1.0")))
BATTERY_FALL_STEP_PERCENT = max(0.1, float(os.getenv("BATTERY_FALL_STEP_PERCENT", "3.0")))
...
ARUCO_DETECTION_MAX_AGE_SEC = float(os.getenv("ARUCO_DETECTION_MAX_AGE_SEC", "5.0"))
ARUCO_DOCKING_TIMEOUT_SEC = float(os.getenv("ARUCO_DOCKING_TIMEOUT_SEC", "35.0"))
...
NAV_GOAL_XY_TOLERANCE_M = float(os.getenv("NAV_GOAL_XY_TOLERANCE_M", "0.02"))
...
NAV_APPROACH_ROTATE_MAX_SEC = float(os.getenv("NAV_APPROACH_ROTATE_MAX_SEC", "45.0"))
...
# Nav2 주행 중 전방 가드: 목표 근처(회전/정밀 접근)에서는 생략
NAV_FORWARD_GUARD_NEAR_GOAL_M = float(os.getenv("NAV_FORWARD_GUARD_NEAR_GOAL_M", "0.25"))
...
LEAVE_DOCK_REVERSE_DISTANCE_M = float(os.getenv("LEAVE_DOCK_REVERSE_DISTANCE_M", "0.35"))
LEAVE_DOCK_MAX_DURATION_SEC = float(os.getenv("LEAVE_DOCK_MAX_DURATION_SEC", "10.0"))
# leave_dock 후진 전 후방 라이다 클리어런스 안전체크
LEAVE_DOCK_CLEARANCE_MARGIN_M = float(os.getenv("LEAVE_DOCK_CLEARANCE_MARGIN_M", "0.15"))
