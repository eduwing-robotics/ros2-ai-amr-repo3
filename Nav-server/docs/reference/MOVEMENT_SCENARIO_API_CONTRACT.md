...
- callback `sequence`는 command별 0부터 1씩 증가하며 `last_completed_step_index`는 감소하지 않는다.
...
위 API가 다른 진단 용도로 남아 있더라도 자동 입출고 실행·재고·화물 상태의 정본으로 사용하지 않는다.
