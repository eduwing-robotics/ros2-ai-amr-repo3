...
#   scripts/start_all_tb3_1.sh tmux       # tmux 모드
#   scripts/start_all_tb3_1.sh windows      # gnome-terminal 개별창
#
...
SESSION="${SESSION:-tb3_1_stack}"
DOMAIN="${DOMAIN:-2}"
MAP="${MAP:-$ROOT/map/robot2_map.yaml}"
# 기본: 대기1 approach (대략 대기장). 완전 hold 삽입 후면 INIT_POSE=dock
# dock: 0.765 0.33 -1.57
...
set +e
echo "========================================"
echo "  ROBOT1 | detector1 (ArUco) | domain $DOMAIN | :8001"
echo "  log: $LOG_DIR/detector1_tb3_1.log"
echo "========================================"
while true; do
  ...
  {
    echo "===== \$(date -Is) detector1 start ====="
    exec env START_CAMERA_LAUNCH=0 ROBOT_ID=tb3_burger_01 scripts/run_pi_camera_aruco.sh
  } 2>&1 | tee -a "\$DETECTOR_LOG" || true
  echo '[detector1] 프로세스 종료 — 5s 후 재시작' | tee -a "\$DETECTOR_LOG"
  sleep 5
done
EOF
}
...
exec env ONLY_ROBOT=tb3_1 PROJECT_VENV='$PROJECT_VENV' \
...
stop_local_stack() {
  log "Nav PC 스택 종료 중 (tb3_1 / domain $DOMAIN only)..."
  ONLY_ROBOT=tb3_1 "$SCRIPT_DIR/start_nav_servers.sh" stop 2>/dev/null || true
  pkill -f "run_nav2_with_initial_pose.sh --robot tb3_1" 2>/dev/null || true
  pkill -f "pane_detector1.sh" 2>/dev/null || true
  pkill -f "detector1_tb3_1" 2>/dev/null || true
  # ArUco / relay for robot1 only
  pkill -f "detection_topic:=/mission/tb3_1/aruco" 2>/dev/null || true
  pkill -f "output_topic:=/mission/tb3_1/camera" 2>/dev/null || true
  pkill -f "ROBOT_ID=tb3_burger_01 scripts/run_pi_camera_aruco" 2>/dev/null || true
  rm -f "$LOG_DIR/detector1_window.pid" 2>/dev/null || true
...
stop_terminator() {
  ...
  pkill -f "terminator -u -g /tmp/tb3_1_term" 2>/dev/null || true
  rm -f "$SESSION_MARKER"
}
...
start_stack_tmux() {
  ...
  if [[ "$WITH_ROBOT" == "1" ]]; then
    ...
    detector_sh="$(write_run_script detector1 "$(cmd_detector1)")"
    ...
    new_win "detector1" "$detector_sh"
  else
    tmux new-session -d -s "$SESSION" -n "detector1" \
      "bash -lc '$(write_run_script detector1 "$(cmd_detector1)"); echo; echo [detector1] 종료; read'"
  fi
  new_win "nav2-rviz" "$(cmd_nav2_rviz)"
  sleep 2
  new_win "nav-servers" "$(cmd_nav_servers)"
  new_win "status" "$(cmd_status)"
  log "attach: scripts/start_all_tb3_1.sh attach"
}
...
start_windows() {
  ...
  if [[ "$WITH_ROBOT" == "1" ]]; then
    ...
    open_win "detector1" "$(write_run_script detector1 "$(cmd_detector1)")"
  else
    open_win "detector1" "$(write_run_script detector1 "$(cmd_detector1)")"
  fi
...
start_terminator() {
  ...
  log "terminator split 시작 ROBOT1 WITH_ROBOT=$WITH_ROBOT WITH_LIFT=$WITH_LIFT WITH_EKF=$WITH_EKF DOMAIN=$DOMAIN"

  local tmpd; tmpd="$(mktemp -d /tmp/tb3_1_term.XXXXXX)"
  ...
  if [[ "$WITH_ROBOT" == "1" ]]; then
    add_pane "R1-bringup" "$(ssh_robot_bringup_body)"
    if [[ "$WITH_LIFT" == "1" ]]; then
      add_pane "R1-lift" "$(ssh_robot_lift_body)"
    fi
    add_pane "R1-camera" "$(ssh_robot_camera_body)"
    add_pane "R1-detector" "$(cmd_detector1)"
  else
    add_pane "R1-detector" "$(cmd_detector1)"
  fi
  add_pane "R1-nav2-rviz" "$(cmd_nav2_rviz)"
  add_pane "R1-api-8001" "sleep 3; $(cmd_nav_servers)"
  add_pane "R1-status" "$(cmd_status)"
...
lines = [
    ...
    "      size = 1680, 1000",
    "      maximised = True",
    '      title = ROBOT1 | tb3_1 | :8001 | domain2',
...
if has_lift:
    term("term_bringup", "robot_row", 0, "R1-bringup", need("R1-bringup"))
    term("term_lift", "robot_row", 1, "R1-lift", need("R1-lift"))
else:
    term("term_bringup", "left_col", 0, "R1-bringup", need("R1-bringup"))

# vision row — camera | detector 나란히
term("term_camera", "vision_row", 0, "R1-camera", need("R1-camera"))
term("term_detector", "vision_row", 1, "R1-detector", need("R1-detector"))

# right column
term("term_nav2", "right_col", 0, "R1-nav2-rviz", need("R1-nav2-rviz"))
term("term_navsrv", "nav_mid", 0, "R1-api-8001", need("R1-api-8001"))
term("term_status", "nav_mid", 1, "R1-status", need("R1-status"))
...
PY
  ...
  if kill -0 "$tpid" 2>/dev/null; then
    ...
    log "레이아웃: 왼쪽 아래 vision_row = robot-camera | detector1 (나란히)"
...
status_stack() {
  ...
  if [[ "${aruco_pub:-0}" -ge 1 ]]; then
    ...
    echo "없음 — detector1 pane 확인 (scripts/nav_ops.sh detector1)"
  ...
  echo "===== 세션 ====="
  if pgrep -f "aruco_detector_node.py" >/dev/null 2>&1; then
    echo "detector1: aruco_detector_node 실행 중"
  else
    echo "detector1: 없음 — robot-camera 오른쪽 pane 또는 fallback 로그 확인"
  fi
  if [[ -f "$SESSION_MARKER" ]]; then
    # shellcheck source=/dev/null
    source "$SESSION_MARKER" 2>/dev/null || true
    if [[ -n "${TERMINATOR_TMPD:-}" && -d "$TERMINATOR_TMPD" ]]; then
      ...
      echo "terminator panes: ${pane_up}/${pane_total} 실행 (detector1 = robot-camera 오른쪽 pane)"
      if [[ "$pane_up" -lt "$pane_total" ]]; then
        echo "  → pane 누락 시: scripts/start_all_tb3_1.sh restart"
      fi
    fi
    echo "terminator: $(tr '\n' ' ' <"$SESSION_MARKER")"
  else
